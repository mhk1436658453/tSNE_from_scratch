...
def pairwise_prob(x, perplexity):
    dist = scipy.spatial.distance.pdist(x, 'sqeuclidean')
    n = np.size(x, 0)
...
def lowdim_prob(y, dist):
    start_time = time.time()
    temp_dist = 1/(1+dist**2)
    dist_sum = temp_dist.sum()

    q = np.zeros(temp_dist.size)

    for c, d in zip(range(temp_dist.size), temp_dist):
        q[c] = d/dist_sum

    print('compute q: {} seconds'.format(time.time() - start_time))
    return q

#def get_grad(p_diff, dist)
...
def t_SNE(x, perplexity, iteration, lr, alpha):
    ...
    for t in range(iteration):
        dist = scipy.spatial.distance.pdist(y[-1])
        q = lowdim_prob(y[-1], dist)
        #
        # gradient = get_grad(p-q, dist)
        #
        # gradient = 4*(p-q)(dist)
        #
        #
